...
pub type CommandHandler = Box<dyn Fn(Arc<Context>, Message, Parser) -> CommandResultOuter + Send + Sync>;
...
impl CommandNode {
    ...
    pub fn get_name(&self) -> &str {
        match &self {
            ...
            CommandNode::GroupNode {name, handler, sub_nodes}  => &name
        }
    ...
    pub fn get(&self, target: &str)-> Option<&CommandNode> {
        match &self {
            ...
            CommandNode::GroupNode {name, handler, sub_nodes} => sub_nodes.get(target)
        }
    }
    ...
    pub async fn execute(&self, ctx: Arc<Context>, msg: Message, parser: Parser) -> CommandResult {
        match &self {
            ...
            CommandNode::GroupNode {name, handler, sub_nodes}  => {
                match handler {
                    Some(handler) => {
                        let test = handler;
                        test(ctx, msg, parser).await?;
